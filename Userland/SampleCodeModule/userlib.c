...
void mem_status() {
    ...
    if (sys_mem_info(&info) == 0) {
        ...
        prints("Total memory: ", 100);
        printDec(info.total_memory);
        prints(" bytes\n", 100);
        
        prints("Used memory: ", 100);
        printDec(info.used_memory);
        prints(" bytes\n", 100);
        
        prints("Free memory: ", 100);
        printDec(info.free_memory);
        prints(" bytes\n", 100);
        
        prints("Allocated blocks: ", 100);
        printDec(info.allocated_blocks);
        prints("\n", 100);
        
        prints("Free blocks: ", 100);
        printDec(info.free_blocks);
        prints("\n", 100);
        
        uint64_t used_percent = (info.used_memory * 100) / info.total_memory;
        prints("Usage: ", 100);
        printDec(used_percent);
        prints("%\n", 100);
        prints("====================\n", 100);
    } else {
        prints("Error getting memory info\n", 100);
    }
}

void test_memory_manager() {
    prints("=== Memory Manager Test ===\n", 100);
    
    // Test 1: Asignar algunos bloques
    prints("Test 1: Basic allocation\n", 100);
    void* ptr1 = malloc(100);
    void* ptr2 = malloc(200);
    void* ptr3 = malloc(50);
    
    prints("Allocated ptr1=0x", 100);
    printHex((uint64_t)ptr1);
    prints(", ptr2=0x", 100);
    printHex((uint64_t)ptr2);
    prints(", ptr3=0x", 100);
    printHex((uint64_t)ptr3);
    prints("\n", 100);
    
    mem_status();
    
    // Test 2: Liberar bloque del medio
    prints("\nTest 2: Free middle block\n", 100);
    free(ptr2);
    mem_status();
    
    // Test 3: Asignar en espacio liberado
    prints("\nTest 3: Allocate in freed space\n", 100);
    void* ptr4 = malloc(150);
    prints("Allocated ptr4=0x", 100);
    printHex((uint64_t)ptr4);
    prints("\n", 100);
    mem_status();
    
    // Test 4: Liberar todos los bloques
    ...
    // Test 5: Asignacion grande
    prints("\nTest 5: Large allocation\n", 100);
    void* big_ptr = malloc(10000);
    if (big_ptr) {
        prints("Large allocation successful: 0x", 100);
        printHex((uint64_t)big_ptr);
        prints("\n", 100);
        free(big_ptr);
    } else {
        prints("Large allocation failed\n", 100);
    }
